...
echo -e "This tool verifies upgrade signatures that are listed in accounts.js (upgradeSignatures)"; 
echo -e "Please follow the instructions to proceed.";
echo -e "First, recreate the upgrade paramaters:";
...
then
  ...
  read input
  node verify-upgrade-signatures.js $network $upgradeType $input
fi 
if [ $upgradeType == "2" ] 
then
  ...
  read input2
  node verify-upgrade-signatures.js $network $upgradeType $input $input2
fi 
if [ $upgradeType == "3" ] 
then
