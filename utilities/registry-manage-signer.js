...
async function main() {
  ...
  if (programArguments.length < 5) {
    console.log(
      "Arguments missing. Please invoke the manage-signer.sh tool again."
    );
  }
  const network = programArguments[2];
  const networks = truffleConfig.networks;
  const signerAddress = programArguments[3];
  const enableSigner = parseInt(programArguments[4]);
  ...
  registryNoSigner
    .getUpgradeNonce()
    .then((nonce) => {
      return registryWithSigner.setSigner(
        nonce.toString(),
        signerAddress,
        enableSigner,
